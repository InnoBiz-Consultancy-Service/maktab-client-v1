...
describe("studentLoginSchema", () => {
  it("accepts a valid code", () => {
    expect(
      studentLoginSchema.safeParse({ studentCode: "STU-ABCD2345" }).success,
    ).toBe(true);
  });
  it("rejects a too-short code", () => {
    expect(studentLoginSchema.safeParse({ studentCode: "ab" }).success).toBe(
      false,
    );
  });
});
...
describe("registerParentSchema", () => {
  ...
  it("rejects a short password", () => {
    expect(
      registerParentSchema.safeParse({ ...valid, password: "123" }).success,
    ).toBe(false);
  });

  it("rejects an invalid email", () => {
    expect(
      registerParentSchema.safeParse({ ...valid, email: "nope" }).success,
    ).toBe(false);
  });
});
